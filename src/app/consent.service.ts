...
export class ConsentService extends BaseService {
  ...
  public current: BehaviorSubject<Consent | null> = new BehaviorSubject<Consent | null>(null);
	...
	queryParameters(p_id: string) {
		return `subject=Patient/${p_id}` + `&_sort=${this.order == 'asc' ? '' : '-'}${this.sort}` + `&_count=${this.pageSize}&_getpagesoffset=${this.offset}`;
	}

	index(p_id: string): Observable<Bundle<Consent>> {
		let b = this.http.get<Bundle<Consent>>(this.url() + "?" + this.queryParameters(p_id), { headers: this.backendService.headers() });
		return b;
	}
  ...
  load(id: string) {
    this.http.get<Consent>(this.urlFor(id), { headers: this.backendService.headers() }).subscribe({
      next: d => {
        this.current.next(d);
      }, error: e => {
